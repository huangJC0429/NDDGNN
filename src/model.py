...
from torch_sparse import SparseTensor, fill_diag, set_diag, remove_diag
...
from src.datasets.data_utils import get_norm_adj, tau_softmax, row_l1_normalize
...
class ADiGCNConv(torch.nn.Module):
    ...
    def delechlet_energy(self, adj, x):
        x2 = row_l1_normalize(x)
        e = (self.I + adj) @ torch.multiply(x2, x2) - 2 * (
                    torch.multiply(((self.I + adj) @ x2), x2) - torch.multiply(x2, x2))
        return e
    def deg_filter(self, x, out_nei, in_nei, mask):
        e_out = self.delechlet_energy(self.adj_norm, x)
        e_in = self.delechlet_energy(self.adj_t_norm, x)
        C_out = self.out_filter((-e_out + self.out_deg_enc(self.out_degree)))  # N*1
        C_in = self.in_filter((-e_in + self.in_deg_enc(self.in_degree)))  # N*1
    ...
    def forward(self, x, edge_index):
        if self.adj_norm is None:
            ...
            row = torch.range(0, x.shape[0] - 1).long().to(device)
            self.I = SparseTensor(row=row, col=row, sparse_sizes=(num_nodes, num_nodes))
...
class LightingFullBatchModelWrapper(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        out, C_ins, C_outs = self.model(x, edge_index)
